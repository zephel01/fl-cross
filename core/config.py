...
def save_exclude_providers(providers: list[str], path: Path | None = None) -> None:
    """除外する提供元(provider)リストを config.toml の [filters] に書き戻す。"""
    p = path or CONFIG_PATH
    ...
    while i < len(lines):
        ...
        if line.strip() == "[filters]":
            found = True
            out.append(line)
            # 既存の filters ブロック内の exclude_providers 行を差し替え
            j = i + 1
            wrote = False
            while j < len(lines) and not lines[j].strip().startswith("["):
                s = lines[j].strip()
                if s.startswith("exclude_providers"):
                    if not wrote:
                        out.append(_fmt_providers(providers))
                        wrote = True
                else:
                    out.append(lines[j])
                j += 1
...
def _fmt_providers(providers: list[str]) -> str:
    items = ", ".join('"' + str(x).replace('"', "") + '"' for x in providers)
    return f"exclude_providers = [{items}]"
