...
SOURCES: list[Source] = [
    ...
    Source(
        ...
        search_url="https://freelance.findy-code.io/projects",
        build_search=lambda kw: f"https://freelance.findy-code.io/projects?keyword={_space(kw)}",
        login_required=True,
        parser_key="findy",
        note="ハイスキル・スタートアップ・高リモート率。GitHub実績が評価されやすい。SPA→Chrome連携推奨",
    ),
    Source(
        key="crowdworks_tech",
        name="クラウドワークス テック",
